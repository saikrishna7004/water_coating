...
def process_pdf_files(pdf_files, check, additional_tags):
    processed_data = {}
    for pdf_file in pdf_files:
        with open(pdf_file, 'rb') as file:
            ...
            data = main.extract_data_from_pdf(pdf_reader, check, additional_tags)
...
def upload():
    if request.method == 'POST':
        ...
        additional_tags = request.form.get('additional_tags') or ''
        print("additional_tags", additional_tags, "anthe")
        processed_data = process_pdf_files(pdf_files, ('Packaging' in checked_keys), additional_tags)
